...
class Browser
{
	...
	public function describe()
	{
		...
		$matches = array();
		preg_match('/\s*(\d*)\.?/', $this->getVersion(), $matches);

		if (isset($matches[1])) {
			$main_version = $matches[1];
		} else {
			$main_version = $this->getVersion();
		}


		return $this->getBrowser() . ' ' . $main_version . " on " . $device_type . $this->getPlatform() . " device";
	}
